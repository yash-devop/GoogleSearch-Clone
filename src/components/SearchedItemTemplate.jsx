        ...
        <>
            <div className="flex flex-col py-3 max-w-[700px]">
                <div className="group cursor-pointer" onClick={()=>window.open(data.link)}>
                    ...
                    <div className="group-hover:underline text-xl text-[#180dab] pt-2">
                        {data.title}
                    </div>
                </div>
                <div className="text-sm text-[#4d5156] leading-6 pt-1" dangerouslySetInnerHTML={{__html: data.htmlSnippet}}/>
            </div>
        </>
    )
};
